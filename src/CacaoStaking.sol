...
contract CacaoStaking is ERC20Vote, ReentrancyGuard {
  ...
  /* -------------------------------------------------------------------------- */
  /**
  So the plan right now is to use the fees generated by the protocol to distribute more 
  YUM to stakers, via the APR of the contract, but i am wondering how we deliver the
   extra YUM to the stakers - is it just by transferring YUM to the contract, thereby distributing 
   it to stakers (without minting more vYUM token
   */
  //  Currently the flow works by calling `deposit()` or `mint()` to deposit the YUM into the contract.
  //  The contract expects the caller to transfer an asset to the contract and then mint the equivalent shares to the
  // address of the `receiver` parameter.
  // In your scenario, we could create a function that "airdrops" tokens to stakers by transferring the tokens to the contract
  // and iterate through the addresses of the wanted receivers and mint the equivalent shares to them.
